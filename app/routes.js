...
// router.post('/v5/find-address', function (req, res) {
...
//   if (postcode) {
//     res.redirect('/v5/add-applicant-step1-1')
//   }
//   else {
//     res.redirect('/v5/add-applicant-step1')
//   }
...
router.post('/v5/do-you-live-in-scotland', function (req, res) {

  var scotland = req.session.data['scotland']

  if (scotland === "yes") {
    res.redirect('/v5/applying-from-scotland')
  }
  else if (scotland === "no") {
    res.redirect('/v5/add-applicant-step2')
  }
  else {
    res.redirect('/v5/add-applicant-step1')
  }
...
router.post('/v5/pregnant-or-children', function (req, res) {
  ...
  if (pregnantorchildren) {
    ...
    if (pregnantorchildren === 'pregnant') {
      res.redirect('/v5/add-applicant-step3')
    }
    else if (pregnantorchildren === 'children') {
      res.redirect('/v5/add-applicant-step4')
    }
    else if (pregnantorchildren === 'NONE') {
      res.redirect('/v5/add-applicant-step2-error')
    }
    else if (pregnantorchildren.toString() == 'pregnant,children') {
      res.redirect('/v5/add-applicant-step3')
    }
...
router.post('/v5/date-of-birth', function (req, res) {
  ...
  if (dateofbirthday && dateofbirthmonth && dateofbirthyear) {
      ...
      if (yrs >= 16 && yrs < 18) {
        res.redirect('/v5/do-they-get-vouchers')
      } else {
        res.redirect('/v5/not-currently-eligible-dob')
      }
...
// Do they get Healthy Start vouchers at the moment?
router.post('/v5/do-they-get-vouchers', function (req, res) {

  var vouchers = req.session.data['vouchers']

  if (vouchers === "yes") {
    res.redirect('/v5/already-get-vouchers')
  }
  else if (vouchers === "no") {
    res.redirect('/v5/do-they-live-in-scotland')
  }
  else {
    res.redirect('/v5/do-they-get-vouchers')
  }
...
router.post('/v5/do-they-live-in-scotland', function (req, res) {
  ...
  if (scotland === "yes") {
    ...
    res.redirect('/v5/are-they-pregnant')
  }
  else {
    res.redirect('/v5/do-they-live-in-scotland')
  }
...
router.post('/v5/are-they-pregnant', function (req, res) {

  var pregnant = req.session.data['pregnant']

  if (pregnant === "yes") {
    res.redirect('/v5/due-date')
  }
  else if (pregnant === "no") {
    res.redirect('/v5/not-currently-eligible')
  }
  else {
    res.redirect('/v5/are-they-pregnant')
  }
...
router.post('/v5/due-date', function (req, res) {
  ...
  if (duedateday && duedatemonth && duedateyear) { // If a date has been entered...
    ...
    if (duedate > fourtytwoweeksfromtoday) { // If due date is greater than 42 weeks from today...
      res.redirect('/v5/due-date-42-weeks') // ...redirect to error screen...
    } else {
      res.redirect('/v5/name') // ...otherwise allow them to continue their journey!
    }
...
router.post('/v5/children-under-four', function (req, res) {
  ...
  if (pregnant === "yes" && childrenunderfour === "no") {
    ...
    res.redirect('/v5/children-under-four')
  }
...
router.post('/v5/address', function (req, res) {
  ...
  if (addressline1 && towncity && postcode) {
    res.redirect('/v5/national-insurance-number')
  }
  else {
    res.redirect('/v5/address')
  }
...
router.post('/v5/terms-and-conditions', function (req, res) {
  ...
  if (terms) {
    ...
    if (duedate > fulltermpregnancy) {
      res.redirect('/v5/confirmation-under-10-weeks')
    } else {
...
router.post('/v5/eligibility', function (req, res) {
  ...
  if (vouchers === "no" && scotland === "no" && pregnant === "yes" && yrs >= 16 && yrs < 18) {
    res.redirect('/v5/details')
  }
  else {
    res.redirect('/v5/eligibility-error')
  }
...
router.post('/v5/details', function (req, res) {
